...
def setup_logger(name="Base", filename="logs", detail="info"):
    ...
    file_handler = SessionFileHandler(f"BitText/logs/{filename}.log")
...
class CustomFormatter(logging.Formatter):
    ...
    RENAME_MAP = {
        "uvicorn.error": "API.API",
        "uvicorn.access": "API.Done",
        "uvicorn": "API"
    }
    ...
    def format(self, record):
        ...
        try:
            ...
            pass  # Keep default width if can't detect terminal size
        ...
        for i, line in enumerate(lines):
            if self.use_color:
                level = f"{color}{level_label:<4}{Style.RESET_ALL}" if record.levelno < logging.ERROR else f"{Fore.RED}{level_label:<4}{Style.RESET_ALL}"
            else:
...
LOGGING_CONFIG = {
    ...
    "handlers": {
        ...
        "file": {
            ...
            "formatter": "custom",
            "filename": "BitText/logs/logs.log",
            "mode": "a",
            "level": "DEBUG",
        },
    },
